...
# import datasets
data_demographic = pd.read_csv(data_dir + "training_subject_specimen.tsv", sep="\t")
data_pbmc = pd.read_csv(data_dir + "training_pbmc_cell_frequency_wide.tsv", sep="\t")

# list of columns without subject id -> list of PBMC types considered
pbmc_cols = data_pbmc.columns[1:].to_list()
...
data = (data.merge(data_pbmc, left_on="specimen_id", right_on="specimen_id")
       .reindex(columns=data.columns.to_list() + data_pbmc.columns[1:].to_list()))
...
## TODO ALTERNATIVE: rename all columns to preserve all data (requires slightly more work)
...
final_cols = (["subject_id", "specimen_id_D0", "specimen_id_D1", "infancy_vac", "biological_sex", "age"] + pbmc_cols + ["Monocytes_D1"])
...
# write to tsv
combined.to_csv("./demographic_PBMCs_wide.tsv", sep="\t", index=False)